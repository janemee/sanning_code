...
public class KuaiShouLiveHotSubTaskJsonController extends BaseController {
    ...
    private static final Logger log = LoggerFactory.getLogger(KuaiShouLiveHotSubTaskJsonController.class);
    ...
    public ResultEntity addSuperHeat(TaskAdminPramsModel taskAdminPramsModel, Task task) throws BussinessException {
        ...
        try {
            String linkUrl = TaskUtil.saveLiveInContentKwaiFu(taskAdminPramsModel.getLiveInContent());
            //检查直播链接是否正确
            if (StringUtil.isNotBlank(taskAdminPramsModel.getLiveInContent()) && StringUtil.isBlank(linkUrl)) {
            ...
            task.setAnalysisContent(linkUrl);
